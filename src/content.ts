...
async function getVideoTranscript(): Promise<string> {
  try {
    ...
    const transcriptButton = document.querySelector('[aria-label*="transcript" i], [aria-label*="captions" i]') as HTMLElement;
    ...
    const transcriptItems = document.querySelectorAll('#transcript .ytd-transcript-segment-renderer');
